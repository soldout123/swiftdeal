...
class ViewController: UIViewController, WKNavigationDelegate, UIDocumentInteractionControllerDelegate {
    ...
    override func viewDidLayoutSubviews() {
        ...
        SwiftDeal_AISmartRide.webView.frame = calcWebviewFrame(webviewView: webviewView, toolbarView: nil)
    }
    
    @objc func keyboardWillHide(_ notification: NSNotification) {
        SwiftDeal_AISmartRide.webView.setNeedsLayout()
    }
    
    func initWebView() {
        SwiftDeal_AISmartRide.webView = createWebView(container: webviewView, WKSMH: self, WKND: self, NSO: self, VC: self)
        webviewView.addSubview(SwiftDeal_AISmartRide.webView);
        
        SwiftDeal_AISmartRide.webView.uiDelegate = self;
        
        SwiftDeal_AISmartRide.webView.addObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress), options: .new, context: nil)

        if(pullToRefresh){
            let refreshControl = UIRefreshControl()
            refreshControl.addTarget(self, action: #selector(refreshWebView(_:)), for: UIControl.Event.valueChanged)
            SwiftDeal_AISmartRide.webView.scrollView.addSubview(refreshControl)
            SwiftDeal_AISmartRide.webView.scrollView.bounces = true
        }

        if #available(iOS 15.0, *), adaptiveUIStyle {
            themeObservation = SwiftDeal_AISmartRide.webView.observe(\.themeColor) { [unowned self] webView, _ in
                let backgroundColor = SwiftDeal_AISmartRide.webView.underPageBackgroundColor;
                let themeColor = SwiftDeal_AISmartRide.webView.themeColor;
    ...
    @objc func refreshWebView(_ sender: UIRefreshControl) {
        SwiftDeal_AISmartRide.webView?.reload()
...
//        toolbarView.autoresizingMask = [.flexibleTopMargin, .flexibleRightMargin, .flexibleWidth]
    ...
    func overrideUIStyle(toDefault: Bool = false) {
        if #available(iOS 15.0, *), adaptiveUIStyle {
            if (((htmlIsLoaded && !SwiftDeal_AISmartRide.webView.isHidden) || toDefault) && self.currentWebViewTheme != .unspecified) {
                UIApplication
                    .shared
                    .connectedScenes
    ...
    @objc func loadRootUrl(cachePolicy: NSURLRequest.CachePolicy = .useProtocolCachePolicy) {
        SwiftDeal_AISmartRide.webView.load(URLRequest(url: SceneDelegate.universalLinkToLaunch ?? SceneDelegate.shortcutLinkToLaunch ?? rootUrl, cachePolicy: cachePolicy))
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!){
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            SwiftDeal_AISmartRide.webView.isHidden = false
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (keyPath == #keyPath(WKWebView.estimatedProgress) &&
                SwiftDeal_AISmartRide.webView.isLoading &&
                !self.loadingView.isHidden &&
                !self.htmlIsLoaded) {
                    var progress = Float(SwiftDeal_AISmartRide.webView.estimatedProgress);
    ...
    deinit {
        SwiftDeal_AISmartRide.webView.removeObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress))
    }
}
...
extension ViewController: WKScriptMessageHandler {
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "print" {
            printView(webView: SwiftDeal_AISmartRide.webView)
        }
